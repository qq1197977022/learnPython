...
class Hero(object):
    ...
    # def boom(self, enemy):
    #     if self.hero_position_lst[0] == enemy.bullet_lst[0].bullet_position_lst[0]:
    #         print('*' * 50)
    #         print('*' * 50)
    #         print('*' * 50)
    #         print('*' * 50)
...
def boom(hero, enemy):
    f = 0
    if hero.bullet_lst:
        if hero.bullet_lst[0].bullet_position_lst[0] - enemy.enemy_position_lst[0] < 10 and hero.bullet_lst[0].bullet_position_lst[1] - enemy.enemy_position_lst[1] < 10:
            # enemy.surf_enemy = pygame.image.load('img/bullet2.png')
            f += 1
            if f == 1:
                enemy.surf_enemy = pygame.image.load('img/enemy0_down1.png')
                f += 1
            if f == 2:
                enemy.surf_enemy = pygame.image.load('img/enemy0.png')
                f += 1
            if f == 3:
                enemy.surf_enemy = pygame.image.load('img/enemy0_down3.png')
    else:
        pass
...
# python中没有私有,但却又有两种类型的私有
# 1.约定俗成的私有: 以_开头
# 2.类私有成员__name_,最少两个_开头,最多一个_结尾,python对此提供了有限的支持 ---> 名称变形
